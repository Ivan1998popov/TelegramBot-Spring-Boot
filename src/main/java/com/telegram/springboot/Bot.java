...
public class Bot extends TelegramLongPollingBot {
    ...
    public void onUpdateReceived(Update update) {
        ...
        if (message!=null&&message.hasText()){
            ...
            }else if(!em.createQuery("SELECT c  FROM Schedule c "
                    +message.getText(),Schedule.class)
                    .getResultList().isEmpty()) {
                sendMsg(message,em.createQuery("SELECT c grope_,data_  FROM Schedule c WHERE CONCAT(grope,' ',data) = "
                        +message.getText(),Schedule.class).getResultList().toString());
            }else {
